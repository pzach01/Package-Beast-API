...
class Subscription(models.Model):
    ...
    def getItemsUsed(self):
        return Item.objects.filter(owner=self.owner, arrangement__isnull=True, shipment_isnull=True).count()
    
    def getContainersUsed(self):
        return Container.objects.filter(owner=self.owner, arrangement__isnull=True, shipment_isnull=True).count()
